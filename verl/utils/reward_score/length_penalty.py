...
def compute_length_reward_traineff(data: DataProto, all_score_lst: List[dict], len_coef=0.2, epsilon=1e-6, **kwargs):
    
    score_lst = [score_dict['score'] for score_dict in all_score_lst]
    correctness_lst = [score_dict['acc'] for score_dict in all_score_lst]

    id2length = defaultdict(list)   # uid 2 correct res len 
    id2mean = {}
    id2std = {}
