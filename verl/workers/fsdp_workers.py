...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                output, entropys, step_scores = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            if self.config.length_penalty.enable_fgreward:
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropys": entropys, "step_scores": step_scores},
